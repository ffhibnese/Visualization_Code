...
legend_font = font_manager.FontProperties(weight='normal', style='normal', size=18)
# legend_font2 = font_manager.FontProperties(fname="Times/Times-New-Romance.ttf", weight='normal', style='normal', size=24)
legend_font2 = font_manager.FontProperties(weight='normal', style='normal', size=25)
...
legend_font4 = font_manager.FontProperties(weight='normal', style='normal', size=20)
...
# class_id = ['62', '715', '150']
lambda_vals = ['Greedy', 'CMI-VLD$_{t}$', 'CMI-VLD$_{v}$', 'CMI-VLD']
# incv3_class_id = ['62', '150', '952']


varying_lambda = [78.4, 79.06, 81.31, 82.18]
...
if __name__ == '__main__':
    ...
    # plt.xlabel('Mask Ratio $\gamma$', fontproperties=legend_font2)
    plt.ylabel('F1 score', fontproperties=legend_font2)

    x = np.linspace(0, 3, 4)
    width = 0.5
    plt.grid(axis = 'y', alpha=0.2)
    plt.xticks(x + width/2, lambda_vals, font=legend_font)
    plt.yticks(range(78, 84), font=legend_font4)
    plt.ylim(ymin = 77.5, ymax=82.3)
    plt.xlabel('Method', font=legend_font4)
    ...
    plt.bar(x + width/2, varying_lambda, width=width, label='Random', color='salmon', alpha=0.99, zorder=10)
    ...
    plt.savefig('../imgs/CMI_VLD_technique_bar.pdf', dpi=400, bbox_inches='tight')
