...
legend_font2 = font_manager.FontProperties(weight='normal', style='normal', size=22)
legend_font3 = font_manager.FontProperties(fname="Times/Times New Roman MT Extra Bold.ttf", weight='semibold', style='normal', size=23)
# legend_font4 = font_manager.FontProperties(fname="Times/Times-New-Romance.ttf", weight='normal', style='normal', size=20)
legend_font4 = font_manager.FontProperties(weight='normal', style='normal', size=23)
legend_font5 = font_manager.FontProperties(fname="Times/Times-New-Romance.ttf", weight='normal', style='normal', size=20)
legend_font6 = font_manager.FontProperties(fname="Times/Times New Roman MT Extra Bold.ttf", weight='semibold', style='normal', size=21)
label_fontdict = {'fontsize': 28}

# dataset_label = ['Average(ImageNet)', 'Average(FFHQ)']
# class_id = ['62', '715', '150']
lambda_vals = ['60%', '70%', '80%', '90%', '100%']
# incv3_class_id = ['62', '150', '952']


varying_lambda = [1466.83, 1466.95, 1481.17, 1460.79, 1450.35]
# varying_lambda = [100 * item for item in varying_lambda]
...
if __name__ == '__main__':
    ...
    plt.xlabel('Retaining Ratio $\gamma$', fontproperties=legend_font2)
    plt.ylabel('MME scores', fontproperties=legend_font2)

    x = np.linspace(0, 4, 5)
    width = 0.6
    plt.grid(axis = 'y', alpha=0.2)
    plt.xticks(x + width/2, lambda_vals, font=legend_font4)
    plt.yticks((1440, 1450, 1460, 1470, 1480), font=legend_font4)
    plt.ylim(ymin = 1440, ymax=1483)
    ...
    # plt.bar(x + width/2, varying_lambda, width=width, label='Random', alpha=0.95, zorder=10)
    plt.bar(x + width/2, varying_lambda, width=width, label='Random', color='royalblue', alpha=0.99, zorder=10)
    # plt.bar(x + 2 * width, avg, width=width, label="All", color='darkseagreen', alpha=0.95, zorder=10)
    # plt.bar(x + 3 * width, Badrdm, width=width, label="Ours", color='orange', alpha=0.95, zorder=10)
    ...
    plt.savefig('../imgs/CMI_VLD_mask_ratio_bar.pdf', dpi=400, bbox_inches='tight')
