...
if __name__ == '__main__':
    ...
    plt.xlabel('Retrieval Number', fontproperties=legend_font2)
    ...
    p4,= plt.plot(x, knn_asr, '*-',markersize=dot_size, color='red', linewidth=line_width, alpha=alpha_1, label='ASR')
    p2,=plt.plot(x, knn_clip_attack, 'p-',markersize=dot_size, color='seagreen', linewidth=line_width, alpha=alpha_2, label='CLIP-Attack')
    p3,= plt.plot(x, knn_clip_benign, 'x-',markersize=15, color='violet', linewidth=line_width, alpha=alpha_3, label='CLIP-Benign')
   
    

    # p5, = plt.plot(x, GIFD, 's-',markersize=9, color='#0504aa', linewidth=2.5, alpha=1, label='GIFD')
    plt.xlim(xmin = 0.5, xmax=6.4)
    ...
    fig.legend(loc='upper center', ncol=4, fontsize=24, bbox_to_anchor=(2.40, 0.535), prop = {'size':24})
    ...
    plt.savefig('../imgs/BadRDM_ablation.pdf', dpi=400, bbox_inches='tight')
