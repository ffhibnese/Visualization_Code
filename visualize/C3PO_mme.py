...
mme_data = [1687.85, 1687.76, 1688.81, 1690.96]
...
def main():
    ...
    bars = ax.bar(x, mme_data, width=bar_width, color=colors, 
                  edgecolor="black", linewidth=2.0, alpha=0.9)

    for bar in bars:
        pass

    ax.set_ylabel("MME Score", fontproperties=_font(31))
    ax.set_xticks(x)
    ...
    fig.savefig(os.path.join(out_dir, "mme_bar.png"), dpi=300, bbox_inches='tight')
    fig.savefig(os.path.join(out_dir, "mme_bar.pdf"), dpi=300, bbox_inches='tight')
    plt.close(fig)
